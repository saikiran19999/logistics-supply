            ...
            steps {
                script {
                    ...
                    sshagent(['prod_ssh_key_id']) {
                        ...
                        sh "ssh -o StrictHostKeyChecking=no ec2-user@${AWS_INSTANCE_IP} 'docker exec mysqli mysql -u${MYSQL_USER} -p${MYSQL_PASSWORD} ${MYSQL_DATABASE} < cms_db.sql' > import_log.txt"
                    }
                }
            }
        }
