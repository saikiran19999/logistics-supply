        ...
        script {
          sshagent(['prod_ssh_key_id']) {
            withCredentials([
              ...
              def mysqlCommand = "mysql -h -u root -p root -e \"ALTER USER 'sai' IDENTIFIED WITH mysql_native_password BY 'sai';\""

              sh "ssh -o StrictHostKeyChecking=no ec2-user@${AWS_INSTANCE_IP} 'mysqlCommand'"

              sh "scp -o StrictHostKeyChecking=no ./database/cms_db.sql ec2-user@${AWS_INSTANCE_IP}:~/"

              def mysqlImportCommand = "mysql -u root -p root cms_db < cms_db.sql"

              sh "ssh -o StrictHostKeyChecking=no ec2-user@${AWS_INSTANCE_IP} 'mysqlImportCommand'"
