        ...
        script {
          sshagent(['prod_ssh_key_id']) {
            withCredentials([
              ...
              sh "ssh -o StrictHostKeyChecking=no ec2-user@${AWS_INSTANCE_IP} 'docker exec mysql mysql -u root -proot -e \"ALTER USER 'sai' IDENTIFIED WITH mysql_native_password BY 'sai';\"'"

              sh "scp -o StrictHostKeyChecking=no ./database/cms_db.sql ec2-user@${AWS_INSTANCE_IP}:~/"

              sh "ssh -o StrictHostKeyChecking=no ec2-user@${AWS_INSTANCE_IP} 'docker exec mysql mysql -u root -proot cms_db < cms_db.sql'"
